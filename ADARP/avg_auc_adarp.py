"""
Run ADARP/run_adarp.py across seeds and methods, then report mean/std AUC per AL round.

A copy of wesad/avg_auc_wesad.py with the run module swapped and the target-validity
sweep rewritten for ADARP. Nothing in the repo root or in wesad/ is touched.

    python ADARP/avg_auc_adarp.py --users "112,111,102" --pool global --classifier lr

Participants that cannot be evaluated against -- too few button presses to give
train, val and test a stress session each -- are dropped from the target list by
--auto_exclude, not from the training pool. They still contribute windows.
"""
...
def submit_seed_job(repo_root, output_dir, template, exp_name, exp_dir, exp_kwargs):
    ...
    # ADARP AL jobs must invoke ADARP/run_adarp.py (not the BP run.py at repo root)
    # so preprocess_adarp_data.prepare_data is used. ADARP/ is a directory, not a
    # package, so this is a path invocation; run_adarp.py puts both the repo root
    # and its own directory on sys.path itself.
    # sbatch exports the submitting environment by default, but the SSL knobs are
    # written in explicitly so a resubmitted script reproduces the same encoder.
    ssl_env = "".join(
        f'export {name}="{os.environ[name]}"\n'
        for name in ("ADARP_BATCH_SSL", "ADARP_SSL_EPOCHS")
        if name in os.environ
    )
    script = script.replace(
        "python -u refactor_run.py EXPDIR EXPNAME KWARGS",
        'cd "REPOROOT"\nexport BAN_AL_OUTPUT_DIR="OUTPUTDIR"\n' + ssl_env
        + 'python -u ADARP/run_adarp.py "EXPDIR" "EXPNAME" KWARGS',
    )
...
def discover_invalid_users(seeds: list[int], task: str, input_df: str) -> set[str]:
    """Participants that cannot be evaluated against, so the sweep skips them.

    ADARP's usable button presses run from one (107) to fifty-one (112), and a
    participant with two cannot put one in train, one in val and one in test.
    `usable_targets` decides that on the built windows.

    What comes back is a set of invalid *targets*, not pool exclusions: everyone
    rejected here still belongs in the global training pool, because their windows
    are real signal. main() drops them from the user list and leaves
    --exclude_users alone.

    Seeds do not enter into it. The session split is deterministic and time
    ordered, so a participant that cannot be split under one seed cannot under any.
    """
    if task not in ("adarp", "bp"):
        return set()

    import preprocess_adarp_data as prep

    eda, hr, meta = prep.load_windows()
    frame = prep.windows_to_frame(eda, hr, meta)
    splits = prep.split_sessions(frame)

    usable = set(prep.usable_targets(frame, splits))
    invalid = {u for u in splits if u not in usable}

    if invalid:
        print(f"[sweep] cannot be targets (still in the pool): {sorted(invalid)}")
    return invalid
...
def import_run_module(args, repo_root: Path, outdir: str):
    ...
    sys.argv = run_argv
    try:
        # ADARP branch: route to ADARP/run_adarp.py instead of the BP run.py.
        # ADARP/ goes on the path too, since run_adarp imports preprocess_adarp_data
        # flat, the way everything else in that directory does.
        for path in (str(repo_root), str(repo_root / "ADARP")):
            if path not in sys.path:
                sys.path.insert(0, path)
        import run_adarp as run_module
    finally:
...
def run_warmup_one_seed(args, run_module, outdir: str, seed: int, user: str | None = None) -> None:
    ...
    warmup_args = SimpleNamespace(
        ...
        task=args.task,
        participant_id=str(effective_user) if args.task in ("adarp", "bp") else args.participant_id,
    ...
    # Same knobs the in-process path uses, so a warmup job and a local run build
    # the same encoder instead of silently disagreeing about batch size.
    batch_ssl = getattr(run_module, "BATCH_SSL_ADARP", 32)
    ssl_epochs = getattr(run_module, "SSL_EPOCHS_ADARP", 100)

    print(f"[warmup] training encoder for user={effective_user} seed={seed} pool={args.pool} "
          f"batch={batch_ssl} epochs={ssl_epochs}")
...
def prepare_shared_encoders_if_needed(args, run_module, outdir: str, seeds: list[int], repo_root: Path, users: list[str] | None = None) -> None:
    ...
    py_cmd_base = (
        f'cd "{repo_root}" && '
        f'python -u ADARP/avg_auc_adarp.py '
    ...
    for user, seed in warmup_pairs:
        ...
        script_path = tmp_dir / f"slurm_adarp_warmup_{suffix}.sh"
        script_path.write_text(script)
    ...
    print(
        ...
        "Wait until they finish, then rerun this command with --skip_warmup to submit AL jobs."
    )
    raise SystemExit(0)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ADARP: run ADARP/run_adarp.py across seeds and average AUC per AL round.")
    ...
    parser.add_argument("--user", default="112")
    parser.add_argument(
        "--users",
        default=None,
        help="Optional comma- or space-separated ADARP participant IDs (e.g. 112,111,102).",
    )
    parser.add_argument("--participant_id", default=None)
    parser.add_argument("--pool", default="global")
    parser.add_argument("--fruit", default="ADARP")
    parser.add_argument("--scenario", default="stress")
    parser.add_argument("--unlabeled_frac", default="0.0018")  # ~37 labels on the ~20.5k-row global pool
    parser.add_argument("--dropout_rate", default="0.5")
    parser.add_argument("--warm_start", default="0")
    parser.add_argument("--task", default="adarp",
                        help="new_helper.parse_args takes any string; run_adarp.py routes "
                             "to the ADARP prep for 'adarp' or 'bp'.")
    ...
    parser.add_argument("--outdir", default="avg_auc_results_adarp")
    parser.add_argument(
        "--submit",
        dest="submit",
        action="store_true",
...
def main() -> int:
    ...
    # If auto_exclude is on, drop participants that cannot be evaluated against.
    # They are removed from the *target* list only -- --exclude_users is what takes
    # someone out of the training pool, and that stays exactly as the caller set it.
    ...
    if args.auto_exclude and not args.analyze_only:
        ...
        dropped = [u for u in users if u in discovered]
        if dropped:
            print(f"[auto_exclude] not valid targets, skipped: {dropped} "
                  f"(their windows still feed the global pool)")
            users = [u for u in users if u not in discovered]
        if not users:
            raise SystemExit("Every requested participant was rejected as a target.")
        if args.task in ("adarp", "bp"):
            args.participant_id = users[0]
            args.user = str(args.participant_id)

    # avg_auc_adarp.py lives in ADARP/, so repo root is one level up.
    ...
    # Warmup-only mode: train one (user, seed) encoder and exit. Used by the
    # slurm jobs submitted by prepare_shared_encoders_if_needed.
    if args.warmup_only:
