...
def submit_job(output_dir, template, exp_name, exp_kwargs):
    ...
    # exp_dir = os.path.join(output_dir,exp_kwargs["user"], exp_name)
    # Match the structure used in preprocess.py: {user}/{fruit}_{scenario}
    exp_dir = os.path.join(output_dir, exp_kwargs["pool"], exp_kwargs["user"], f"{exp_kwargs['fruit']}_{exp_kwargs['scenario']}", exp_name)
    # Create base experiment directory; hyperparameter folder is added in run.py
