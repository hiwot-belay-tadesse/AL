...
def import_run_module(args, repo_root: Path, outdir: str):
    ...
    try:
        ...
        # WESAD branch: route to wesad/run_wesad.py instead of the BP run.py.
        from wesad import run_wesad as run_module
    finally:
...
def prepare_shared_encoders_if_needed(args, run_module, outdir: str, seeds: list[int], repo_root: Path, users: list[str] | None = None) -> None:
    ...
    py_cmd_base = (
        f'cd "{repo_root}" && '
        f'python -u -m wesad.avg_auc_wesad '
    ...
    for user, seed in warmup_pairs:
        ...
        script_path = tmp_dir / f"slurm_wesad_warmup_{suffix}.sh"
        script_path.write_text(script)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="WESAD: run wesad/run_wesad.py across seeds and average AUC per AL round.")
    ...
    parser.add_argument(
        "--users",
        default=None,
        help="Optional comma- or space-separated WESAD subject IDs (digits only, e.g. 2,3,4,...).",
    )
    parser.add_argument("--participant_id", default=None)
    parser.add_argument("--pool", default="global")
    parser.add_argument("--fruit", default="WESAD")
    parser.add_argument("--scenario", default="stress")
    parser.add_argument("--unlabeled_frac", default="0.05")
    parser.add_argument("--dropout_rate", default="0.5")
    parser.add_argument("--warm_start", default="0")
    parser.add_argument("--task", default="bp",
                        help="Kept as 'bp' so new_helper.parse_args (in run_wesad) accepts it. "
                             "wesad/run_wesad.py routes to the WESAD prep regardless.")
    ...
    parser.add_argument("--outdir", default="avg_auc_results_wesad")
    parser.add_argument(
        "--submit",
        dest="submit",
        action="store_true",
